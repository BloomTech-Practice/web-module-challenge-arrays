...
function getFlavorByIndex(arr, index) {
  const removedFlavor = arr.splice(index, 1);
  return removedFlavor.toString();
}
...
function removeFlavorByName(arr, removedFlavor) {
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] === removedFlavor) {
      arr.splice(i, 1);
    }
  }
  return arr;
...
function filterByWord(arr, filteredWord) {
  const newArr = [];

  for (let i = 0; i < arr.length; i++) {
    if (arr[i].includes(filteredWord)) {
      newArr.push(arr[i]);
    }
  }
  return newArr;
